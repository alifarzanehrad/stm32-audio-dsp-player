...
DSTATUS SD_initialize(BYTE lun)
{
  ...
  /* USER CODE: RTOS scheduler not started in this project, so removed the
     osKernelRunning() check to allow SD init in bare-metal mode. */
  {
...
{
  int status_check = SD_CheckStatusWithTimeout(SD_TIMEOUT);
  printf("SD_read: status_check=%d (0=ready)\r\n", status_check);
  if (status_check < 0)
  {
    printf("SD_read: card not ready, aborting\r\n");
    return RES_ERROR;
  }

  uint8_t read_ret = BSP_SD_ReadBlocks((uint32_t*)buff, sector, count, SD_TIMEOUT);
  printf("SD_read: sector=%lu count=%u BSP_SD_ReadBlocks ret=%d (0=OK)\r\n",
         (unsigned long)sector, (unsigned)count, read_ret);
